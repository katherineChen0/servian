...
If you want, I can add an option to upload recorded audio per word, or package the app as a PWA so you can install it and keep voices cached.

Per-word audio (upload / record)

- The app now supports attaching per-word audio. Use the **Upload** button next to any word to upload an audio file (MP3/WEBM/etc). Uploaded audio is saved in the browser (in `localStorage`) as a data URL.
- You can also record directly from your device microphone using the **Record** button. The browser will ask for microphone permission. After you stop recording the audio is saved to that word and will be used for playback.
- Playback priority: if a word has uploaded/recorded audio that will be played; otherwise the app falls back to `speechSynthesis`.
- Notes and limitations:
  ...
  - Recording requires `getUserMedia` and `MediaRecorder` support (modern Chrome/Edge/Firefox). Mobile Safari may have limitations — record on desktop or Android if needed.
